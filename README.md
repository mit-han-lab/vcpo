...
VCPO is implemented in the Megatron backend, with core logic in [megatron_actor.py](verl/workers/actor/megatron_actor.py), [vcpo.py](verl/workers/utils/vcpo.py), and [staleness_utils.py](recipe/fully_async_policy/staleness_utils.py). Training scripts are under [recipe/fully_async_policy/shell/vcpo/](recipe/fully_async_policy/shell/vcpo/).

**1. Install** — follow the [veRL documentation](https://verl.readthedocs.io/en/latest/start/install.html) to set up the environment. Specifically, we use Megatron-Core 0.13.1 with vLLM 0.11.0.
...
This repository is based on [veRL](https://github.com/volcengine/verl).
