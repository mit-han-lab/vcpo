...
export VERL_DEBUG_MULTITURN="${VERL_DEBUG_MULTITURN:-recipe/fully_async_policy/code_sandbox/traj}"

# ================= Paths =================
MODEL_PATH=${MODEL_PATH:-"${HOME}/async-rl/models/Qwen2.5-3B"}
TRAIN_FILE=${TRAIN_FILE:-"${HOME}/async-rl/data/deepscaler/deepscaler.parquet"}
TEST_FILE="['${HOME}/async-rl/data/aime/aime24.parquet','${HOME}/async-rl/data/aime/aime25.parquet','${HOME}/async-rl/data/amc2025/test.parquet']"
project_name='async_rl_debug'
...
# ================= Logging =================
exp_name="debug-tool-loop qwen2.5-3b turns-${max_user_turns} stale-${staleness_threshold}"
exp_name_safe=${exp_name//\//_}
log_dir="recipe/fully_async_policy/code_sandbox/logs/${exp_name_safe}"
CKPTS_DIR="${log_dir}"
mkdir -p -- "${log_dir}" "${VERL_DEBUG_MULTITURN}"
run_log_file="${log_dir}/output_log.txt"
: > "${run_log_file}"
...
# ================= Run =================
python -m recipe.fully_async_policy.code_sandbox.test_tool_agent_loop \
    --config-path=/home/lukh23/vcpo-dev-v2/recipe/fully_async_policy/config \
