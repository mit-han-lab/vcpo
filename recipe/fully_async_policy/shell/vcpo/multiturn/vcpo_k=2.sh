...
TRAIN_FILE=${TRAIN_FILE:-"data/dapo_filter/train/dapo_filter_with_tool.parquet"}
TEST_FILE=${TEST_FILE:-"data/aime/aime25_with_tool.parquet"}
...
# ================= Logging =================
exp_name="VCPO k-${staleness_threshold} DAPO-AIME Qwen2.5-7B ${n_gpus_rollout}-${n_gpus_training} B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd ${bsz_per_dp_rank}-rollout_bsz"
